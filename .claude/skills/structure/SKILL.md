...
Do not defer the conversion mechanism vaguely to `/build`. A waitlist requires contact details to reach a real destination; a preference test requires a persisted structured response; a booking requires a working booking destination; a purchase or reservation requires a real transaction or reservation system. An external form, email, Instagram DM, WhatsApp conversation, booking page, checkout, or similar destination can qualify when the promised action genuinely reaches and remains with its owner. If the intended conversion is not connected yet, first offer the simplest truthful existing destination: email, Instagram, WhatsApp, a simple form, booking, or checkout. Otherwise default to `Connection required before launch`; do not silently change the objective. Use `Prototype only` only when the participant explicitly requests a presentation-only page after accepting that it records no response and cannot validate demand.

For an explicitly requested `Prototype only` page, end the public journey at a useful on-page destination. Use a specific exploration CTA, request no discarded input, and create no fake validation or confirmation state. State availability naturally once; never publish internal prototype or research labels. Keep the complete approved proposition visible. A participant-approved experiment price uses one natural qualifier such as `Expected price: €12` or `Around €12`; record the evidence status internally. Treat “just fake it” as a request for a non-collecting concept presentation, never as permission to imitate a successful conversion.
...
Require the first screen and closing section to carry the same useful primary action to an exact real destination. An internal scroll may support exploration but cannot be the only primary action for a launch-oriented page. Give a research-critical comparison and reason to believe enough visual and content weight to change the visitor's decision. Keep routine availability answers concise and out of a long FAQ when they fit better beside the offer or CTA.
...
- The primary action goes beyond an internal scroll, appears in the first screen and closing section, and has an exact destination and next-step promise.
