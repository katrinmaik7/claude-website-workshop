...
The live workshop result must lead to one useful real destination. For a pre-launch idea this can be an existing email address, Instagram profile, WhatsApp link, simple form, booking page, or checkout. A button that only scrolls, a closing slogan without an action, or customer-facing labels such as `Concept preview`, `Proposed launch price`, and `there is no list to join` do not count as a completed workshop website.
